...
class ImageDataset(tdv.VisionDataset):
    ...
    def get_sample(self, filename, bb=None, id=None):
...
class ImageFolderDataset(ImageDataset):
    ...
    def _load_annotations(self, split):
        ...
        img_root = self.fullsize_img_dir
        classes, class_to_idx = self._find_classes(img_root)
        samples = folder.make_dataset(img_root, class_to_idx, extensions=folder.IMG_EXTENSIONS)
        self.classes = classes
        self.class_to_ids = class_to_idx
        fnames = [strip_root(s[0], img_root) for s in samples]
